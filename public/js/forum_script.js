$(document).ready(function () {
    $.ajax({
        ...
        success: function (response) {
            ...
            response.data.forEach(function (dep) {
                console.log(response.data);
                var x = verify_function(dep.access, "1");
                console.log(x);
                if (x==0) {
    ...
    $('#dataFilterInput').on('input', function () {
        ...
        $.ajax({
        type: "GET",
        url: 'http://localhost/backend_travel/users/view_post.php',
        data: {
            user_id: user_id,
            searchBar:searchBar
        },
    ...
    $(document).on('click', '.shareButton', function (evt) {
        var post_id = $(this).attr('data-user_id');
        // Now you have both the comment and the user ID
        var user_id = "<?php echo $_SESSION['user_id']; ?>";
        window.location.href = "./share.php?id_post="+post_id;
    });
